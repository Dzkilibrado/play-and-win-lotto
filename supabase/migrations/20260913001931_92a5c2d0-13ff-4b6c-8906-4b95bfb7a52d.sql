ALTER TABLE public.pool_documents ADD COLUMN original_file_name text;
ALTER TABLE public.pool_document_versions ADD COLUMN original_file_name text;
...
BEGIN
  ...
  SET storage_path = _storage_path, mime_type = _mime_type, file_size = _file_size, original_file_name = NULLIF(trim(COALESCE(_original_file_name, '')), ''), version = version + 1
