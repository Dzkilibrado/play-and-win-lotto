CREATE INDEX IF NOT EXISTS draw_numbers_number_draw_idx ON public.draw_numbers USING btree (number, draw_id);
...
DECLARE
  ...
  _rows jsonb;
BEGIN
  CREATE TEMP TABLE IF NOT EXISTS _sd (id uuid) ON COMMIT DROP;
  ...
  WITH base AS (
    ...
    ORDER BY
      ...
      CASE WHEN _sort NOT IN ('oldest','prize_desc','prize_asc','revenue_desc') OR _sort IS NULL
...
REVOKE ALL ON FUNCTION public.search_draws(text, integer, date, date, text, integer[], text, integer, integer) FROM public;
GRANT EXECUTE ON FUNCTION public.search_draws(text, integer, date, date, text, integer[], text, integer, integer) TO authenticated;
...
BEGIN
  ...
  CREATE TEMP TABLE IF NOT EXISTS _cs (id uuid) ON COMMIT DROP;
...
REVOKE ALL ON FUNCTION public.lottery_combination_stats(text, integer[], integer) FROM public;
GRANT EXECUTE ON FUNCTION public.lottery_combination_stats(text, integer[], integer) TO authenticated;